...
exports.gigaChatService = exports.GigaChatService = void 0;
...
class GigaChatService {
    ...
    enabled;
    constructor() {
        this.enabled = config_1.isGigaChatEnabled;
        const httpsAgent = this.createHttpsAgent();
        this.authClient = axios_1.default.create({
            baseURL: config_1.appConfig.GIGACHAT_AUTH_URL,
            httpsAgent,
        });
        this.apiClient = axios_1.default.create({
            baseURL: config_1.appConfig.GIGACHAT_BASE_URL,
            httpsAgent,
        });
    }
    createHttpsAgent() {
        const certPathStr = config_1.appConfig.GIGACHAT_CA_CERT_PATH ?? "";
        const certPaths = certPathStr.split(";").map((p) => p.trim()).filter(Boolean);
        if (certPaths.length === 0) {
            logger_1.logger.warn("Сертификаты CA для GigaChat не указаны, используется системный сертификат", {
                location: "createHttpsAgent",
            });
            return new node_https_1.Agent();
        }
        const caBundle = [];
        for (const certPath of certPaths) {
            const resolvedPath = node_path_1.default.isAbsolute(certPath) ? certPath : node_path_1.default.resolve(process.cwd(), certPath);
            if (!(0, node_fs_1.existsSync)(resolvedPath)) {
                logger_1.logger.error("Сертификат CA GigaChat не найден", { certPath: resolvedPath, location: "createHttpsAgent" });
                continue;
            }
            try {
                caBundle.push((0, node_fs_1.readFileSync)(resolvedPath));
            }
            catch (error) {
                logger_1.logger.error("Не удалось прочитать сертификат CA GigaChat", {
        ...
        if (caBundle.length === 0) {
            logger_1.logger.warn("Не удалось загрузить сертификаты CA для GigaChat, используется системный сертификат", {
                location: "createHttpsAgent",
            });
            return new node_https_1.Agent();
        }
    ...
    async getAccessToken() {
        ...
        try {
            ...
            let authHeader;
            if (config_1.appConfig.GIGACHAT_AUTHORIZATION_KEY) {
                // GIGACHAT_AUTHORIZATION_KEY может содержать base64-закодированную строку или уже готовый "Basic <base64>"
                const authKey = config_1.appConfig.GIGACHAT_AUTHORIZATION_KEY.trim();
                ...
                try {
                    ...
                    logger_1.logger.warn("Не удалось декодировать GIGACHAT_AUTHORIZATION_KEY как base64", {
                        error: decodeError,
            ...
            else if (config_1.appConfig.GIGACHAT_CLIENT_ID && config_1.appConfig.GIGACHAT_CLIENT_SECRET) {
                // Кодируем client_id:client_secret в base64
                const credentials = `${config_1.appConfig.GIGACHAT_CLIENT_ID}:${config_1.appConfig.GIGACHAT_CLIENT_SECRET}`;
            ...
            bodyParams.set("scope", config_1.appConfig.GIGACHAT_SCOPE);
            ...
            requestUrl = "";
            logger_1.logger.debug("GigaChat token request", {
                baseURL: config_1.appConfig.GIGACHAT_AUTH_URL,
                url: requestUrl || "(empty - root)",
                fullUrl: `${config_1.appConfig.GIGACHAT_AUTH_URL}${requestUrl}`,
                method: "POST",
                headers: {
                ...
                body: bodyString,
                scope: config_1.appConfig.GIGACHAT_SCOPE,
                credentialsInfo,
            ...
            if (!token || typeof token !== "string") {
                logger_1.logger.error("GigaChat API не вернул токен", {
            ...
            logger_1.logger.debug("GigaChat token получен", {
            ...
            if (axios_1.default.isAxiosError(error)) {
                ...
                logger_1.logger.error("Ошибка получения токена GigaChat", {
                    status,
                    statusText,
                    statusCode: status,
                    responseHeaders: responseHeaders ? Object.keys(responseHeaders) : undefined,
                    errorData: errorMessage,
                    requestUrl: `${config_1.appConfig.GIGACHAT_AUTH_URL}${requestUrl || ""}`,
                    scope: config_1.appConfig.GIGACHAT_SCOPE,
                    hasAuthKey: Boolean(config_1.appConfig.GIGACHAT_AUTHORIZATION_KEY),
                    hasClientId: Boolean(config_1.appConfig.GIGACHAT_CLIENT_ID),
                    hasClientSecret: Boolean(config_1.appConfig.GIGACHAT_CLIENT_SECRET),
                    credentialsInfo,
                    location: "getAccessToken",
                });
                // Если это 400 Bad Request, возможно проблема в credentials или формате запроса
                if (status === 400) {
                    logger_1.logger.error("Возможные причины ошибки 400:", {
                        reasons: [
                        ...
                        currentScope: config_1.appConfig.GIGACHAT_SCOPE,
                        location: "getAccessToken",
                    });
                }
            }
    ...
    async complete(messages, temperature = 0.2) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                const response = await this.apiClient.post("/chat/completions", {
                    model: config_1.appConfig.GIGACHAT_MODEL,
                    messages,
                    temperature,
                    max_tokens: 2000,
                }, {
                ...
                if (attempt < maxRetries) {
                    ...
                    logger_1.logger.warn(`Ошибка запроса к GigaChat, попытка ${attempt}/${maxRetries}`, {
                        error,
                        location: "complete",
                        attempt,
                    });
        ...
        logger_1.logger.error("Не удалось выполнить запрос к GigaChat после всех попыток", {
            error: lastError,
            location: "complete",
        });
        throw lastError;
    }
    async summarizeChat(chatTitle, preparedMessages, range, options = {}, chatMembers = [], _materials = []) {
        if (!this.enabled) {
            throw new Error("GigaChat integration is not configured");
        }
        const membersInfo = chatMembers.length > 0
            ? [
                "",
                "УЧАСТНИКИ ЧАТА:",
                (0, text_1.formatBulletList)(chatMembers
                    .sort((a, b) => (b.messageCount ?? 0) - (a.messageCount ?? 0))
                    .slice(0, 15)
                    .map((m) => {
                    const parts = [m.name];
                    if (m.username)
                        parts.push(`@${m.username}`);
                    if (m.messageCount && m.messageCount > 0)
                        parts.push(`(${m.messageCount} сообщ.)`);
                    return parts.join(" ");
                })),
            ].join("\n")
            : "";
        ...
        return (0, text_1.truncate)(completion.result.trim(), 2500);
    }
    async extractTasks(messageText, context, existingTasks) {
        if (!this.enabled) {
            return [];
        }
        const basePrompt = [
        ...
        const existingTasksInfo = existingTasks && existingTasks.length > 0
            ? [
                ...
                (0, text_1.formatBulletList)(existingTasks.map((t) => `${t.title}${t.dueDate ? ` (${(0, date_1.formatDate)(t.dueDate)})` : ""}`)),
            ].join("\n")
            : "";
        ...
        if (process.env.LOG_LEVEL === "debug") {
            logger_1.logger.debug("GigaChat extractTasks response", {
        ...
        try {
            ...
            const tasks = (Array.isArray(parsed) ? parsed : []).map((task) => {
                        ...
                        try {
                            ...
                            if (!parsedDate) {
                                ...
                                if (lowerDate === "завтра" || lowerDate === "tomorrow") {
                                    ...
                                    logger_1.logger.warn("GigaChat вернул 'today' вместо относительной даты", {
                            ...
                            if (parsedDate) {
                                ...
                                if (!isNaN(isoDate.getTime())) {
                                    ...
                                    logger_1.logger.warn("Не удалось распарсить дату из GigaChat", {
                                        dueDate: task.dueDate,
                                        location: "extractTasks",
                                    });
                                }
                            ...
                            logger_1.logger.warn("Ошибка при парсинге даты из GigaChat", {
                                error: error instanceof Error ? error.message : String(error),
                                dueDate: task.dueDate,
                                location: "extractTasks",
                            });
            ...
            if (process.env.LOG_LEVEL === "debug" || tasks.length === 0) {
                logger_1.logger.debug("GigaChat extractTasks parsed", {
                    messageText: messageText.substring(0, 200),
            ...
            logger_1.logger.warn("Не удалось распарсить JSON задач из GigaChat", {
                error: error instanceof Error ? error.message : String(error),
                rawResponse: completion.result.substring(0, 500),
                messageText: messageText.substring(0, 200),
                location: "extractTasks",
     ...
     */
    async analyzeMaterial(material, context) {
        if (!this.enabled) {
            return null;
        }
        ...
        try {
            ...
            logger_1.logger.warn("Ошибка анализа материала через GigaChat", {
                error: error instanceof Error ? error.message : String(error),
    ...
    async answerQuestion(question, context, options) {
        ...
        const membersInfo = options?.chatMembers && options.chatMembers.length > 0
            ? [
                ...
                (0, text_1.formatBulletList)(options.chatMembers.map((m) => {
                    const parts = [m.name];
                    if (m.username)
                        parts.push(`@${m.username}`);
                    return parts.join(" ");
    ...
    async checkMessageImportance(messageText) {
        if (!this.enabled) {
            return false;
        }
        const messages = [
        ...
        try {
            ...
            logger_1.logger.warn("Ошибка проверки важности сообщения", { error, location: "checkMessageImportance" });
            return false;
        }
    }
}
exports.GigaChatService = GigaChatService;
exports.gigaChatService = new GigaChatService();
//# sourceMappingURL=gigachatService.js.map