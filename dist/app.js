...
const importantMessageService_1 = require("./services/importantMessageService");
const keyboardService_1 = require("./services/keyboardService");
const messageService_1 = require("./services/messageService");
const preferenceService_1 = require("./services/preferenceService");
const reminderService_1 = require("./services/reminderService");
const scheduledDigestService_1 = require("./services/scheduledDigestService");
const searchService_1 = require("./services/searchService");
const taskService_1 = require("./services/taskService");
const userChatService_1 = require("./services/userChatService");
const date_1 = require("./utils/date");
const ids_1 = require("./utils/ids");
const number_1 = require("./utils/number");
const text_1 = require("./utils/text");
class App {
    bot = new max_bot_api_1.Bot(config_1.appConfig.MAX_BOT_TOKEN);
    welcomeImageToken = null;
    async init() {
        await (0, db_1.connectDatabase)();
        await reminderService_1.reminderService.init(this.handleReminder);
        await scheduledDigestService_1.scheduledDigestService.init(this.bot.api);
        assistantService_1.assistantService.setBotApi(this.bot.api); // Передаем API для получения участников чата
        digestService_1.digestService.setBotApi(this.bot.api); // Передаем API для дайджестов
     ...
     */
    async preloadWelcomeImage() {
        try {
            const imagePath = (0, node_path_1.join)(process.cwd(), "src", "start_photo.png");
            const image = await this.bot.api.uploadImage({
                source: (0, node_fs_1.readFileSync)(imagePath),
            });
            ...
            if (imageJson.type === "image" && "payload" in imageJson && imageJson.payload) {
                const payload = imageJson.payload;
                if (payload.photos) {
                    const firstPhoto = Object.values(payload.photos)[0];
                    if (firstPhoto?.token) {
                        this.welcomeImageToken = firstPhoto.token;
                        logger_1.logger.system("Изображение приветствия предзагружено");
                    }
                }
            }
        }
        catch (error) {
            logger_1.logger.warn("Не удалось предзагрузить изображение приветствия", {
    ...
    async start() {
        try {
            logger_1.logger.system("Запуск бота...");
            ...
            while (botInfoRetries < maxBotInfoRetries) {
                try {
                    ...
                    logger_1.logger.system(`Бот запущен: @${this.bot.botInfo?.username ?? "unknown"}`);
                    break; // Success
                }
                catch (error) {
                    botInfoRetries++;
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    if (errorMessage.includes("fetch failed") || error?.name === "FetchError") {
                        logger_1.logger.warn(`Ошибка сети при получении информации о боте (попытка ${botInfoRetries}/${maxBotInfoRetries})`, {
                            location: "App.start",
                        });
                    ...
                    logger_1.logger.error("Не удалось получить информацию о боте", {
            ...
            this.bot.start().catch((error) => {
                logger_1.logger.error("Ошибка в цикле polling", {
                    location: "start.polling",
                    error,
                });
            });
            logger_1.logger.system("Бот запущен и готов к работе");
            // Give polling a moment to start
            await new Promise((resolve) => setTimeout(resolve, 2000));
        }
        catch (error) {
            logger_1.logger.error("Не удалось запустить бота", {
                location: "start",
                error,
            });
            throw error;
    ...
    async stop() {
        ...
        logger_1.logger.system("Бот остановлен");
    }
    registerHandlers() {
        this.bot.catch(async (error, ctx) => {
            const userId = (0, number_1.toInt)(ctx.user?.user_id);
            logger_1.logger.error("Необработанная ошибка бота", {
                userId,
        ...
        this.bot.use(async (ctx, next) => {
            try {
                ...
                const userId = (0, number_1.toInt)(ctx.user?.user_id);
                logger_1.logger.error("Ошибка обработки обновления", {
                    userId,
        ...
        this.bot.on("bot_started", async (ctx) => {
            try {
                ...
                if (botInfo) {
                    logger_1.logger.system(`Бот готов к работе: @${botInfo.username}`);
                }
            }
            catch (error) {
                logger_1.logger.error("Ошибка в обработчике bot_started (системное сообщение)", {
            ...
            if (!userId) {
                logger_1.logger.warn("Событие bot_started без userId, пропускаем отправку приветствия", {
                    user: ctx.user,
                    location: "bot_started",
                });
                return;
            }
            await preferenceService_1.preferenceService.getOrCreate(userId);
            ...
            if (this.welcomeImageToken) {
                try {
                    const image = new max_bot_api_1.ImageAttachment({ token: this.welcomeImageToken });
                    attachments.push(image.toJson());
                }
                catch (error) {
                    logger_1.logger.warn("Не удалось использовать кэшированное изображение", {
                        userId,
            ...
            const keyboard = keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null);
            ...
            try {
                ...
                logger_1.logger.success("Приветствие отправлено успешно", { userId, userName });
            }
            catch (error) {
                logger_1.logger.error("Ошибка при отправке приветствия", {
                    userId,
                    userName,
            ...
            if (hasStartPayload) {
                logger_1.logger.userAction(userId, "Кнопка 'Начать' нажата через bot_started", {
        ...
        this.bot.command("start", async (ctx) => {
            const userId = ctx.user ? (0, number_1.toInt)(ctx.user.user_id) : null;
            logger_1.logger.command(userId ?? undefined, "start", ctx.chatId);
            await this.handleStartCommand(ctx);
        });
        this.bot.command("help", async (ctx) => {
            const userId = ctx.user ? (0, number_1.toInt)(ctx.user.user_id) : null;
            logger_1.logger.command(userId ?? undefined, "help", ctx.chatId);
            await this.handleHelpCommand(ctx);
        });
        // Используем регулярные выражения для команд с аргументами
        this.bot.command(/^digest(\s|$)/, async (ctx) => this.handleDigestCommand(ctx));
        this.bot.command("deadlines", async (ctx) => this.handleDeadlinesCommand(ctx));
        this.bot.command("calendar", async (ctx) => this.handleCalendarCommand(ctx));
        this.bot.command(/^search(\s|$)/, async (ctx) => this.handleSearchCommand(ctx));
        this.bot.command("materials", async (ctx) => this.handleMaterialsCommand(ctx));
        this.bot.command("tasks", async (ctx) => this.handleTasksCommand(ctx));
        this.bot.command("chats", async (ctx) => this.handleChatsCommand(ctx));
        this.bot.command("select_chat", async (ctx) => this.handleSelectChatCommand(ctx));
        this.bot.command("sync_chats", async (ctx) => this.handleSyncChatsCommand(ctx));
        ...
        this.bot.hears(/^(Начать|начать|START|start)$/i, async (ctx) => {
            ...
            if (!userId)
                return;
            logger_1.logger.debug("Получен текст 'Начать'", {
                userId,
    ...
    async handleStartCommand(ctx) {
        ...
        attachments.push(keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null));
        await ctx.reply(welcomeText, { attachments });
        logger_1.logger.userAction(userId, "Команда /start выполнена", { userName });
    }
    /**
    ...
    isBotMentioned(message, botUserId) {
        ...
        const mention = markup.find((m) => {
            if (m.type === "user_mention") {
                const userId = (0, number_1.toInt)(m.user_id);
    ...
    async handleIncomingMessage(ctx) {
        ...
        try {
            await messageService_1.messageService.upsertFromMaxMessage(message);
        }
        catch (error) {
            logger_1.logger.error("Не удалось сохранить сообщение", {
                location: "handleIncomingMessage.saveMessage",
                userId: (0, number_1.toInt)(message.sender?.user_id),
                chatId: (0, number_1.toInt)(message.recipient.chat_id),
                error,
            });
        }
        ...
        if (!isPersonal) {
            const userId = (0, number_1.toInt)(message.sender?.user_id);
            const chatId = (0, number_1.toInt)(message.recipient.chat_id);
            if (userId && chatId) {
                try {
                    const chatTitle = ctx.chat?.title ?? undefined;
                    await userChatService_1.userChatService.addChat(userId, chatId, chatTitle);
                }
                catch (error) {
                    logger_1.logger.warn("Не удалось автоматически добавить чат", {
        ...
        if (isPersonal) {
            const userId = (0, number_1.toInt)(message.sender?.user_id);
            if (!userId) {
                logger_1.logger.warn("Не удалось определить пользователя", {
            ...
            const existingPreference = await db_1.prisma.userPreference.findUnique({
            ...
            if (!existingPreference) {
                ...
                const keyboard = keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null);
                attachments.push(keyboard);
                logger_1.logger.debug("Отправка приветствия", {
                    userId,
                    attachmentsCount: attachments.length,
                    hasImage: this.welcomeImageToken ? true : false,
                    hasKeyboard: true,
                ...
                await ctx.reply(welcomeText, { attachments });
                logger_1.logger.userAction(userId, "Первое взаимодействие с ботом - показано приветствие", { userName });
                return;
            }
            // Используем активный чат пользователя
            const selectedChatId = await userChatService_1.userChatService.getSelectedChat(userId);
            const chatId = selectedChatId ? (0, number_1.toInt)(selectedChatId) : null;
            // Если чат не выбран, предлагаем выбрать
            if (!chatId) {
                logger_1.logger.userAction(userId, "Задан вопрос без выбранного чата", { question: text.substring(0, 50) });
                const replyText = [
                ...
                await ctx.reply(replyText, { attachments: [keyboardService_1.keyboardService.getChatsMenu()] });
                return;
            }
            logger_1.logger.userAction(userId, "Задан вопрос ассистенту", { chatId, question: text.substring(0, 50) });
            const answer = await assistantService_1.assistantService.answerPersonalQuestion(userId, chatId, text, this.bot.api);
            await ctx.reply(answer.body);
            logger_1.logger.success("Ответ ассистента отправлен", { userId, chatId });
            return;
        }
        ...
        const botInfo = this.bot.botInfo;
        const botUserId = botInfo ? (0, number_1.toInt)(botInfo.user_id) : undefined;
        ...
        if (!isMentioned) {
            ...
            try {
                ...
                await taskService_1.taskService.processIncomingMessage(message);
                // Проверяем важность сообщения (уведомления отправляются в личку, не в чат)
                const importance = await importantMessageService_1.importantMessageService.checkIfImportant(message);
                if (importance.isImportant) {
                    try {
                        ...
                        if (chatId) {
                            ...
                            if (members?.members) {
                                await importantMessageService_1.importantMessageService.notifyUsersAboutImportantMessage(message, members.members
                                    .map((m) => {
                                    const userId = (0, number_1.toInt)(m.user_id);
                                    return userId ? { user_id: userId } : null;
                                })
                                    .filter((m) => m !== null), this.bot.api);
                            }
                        }
                    }
                    catch (error) {
                        logger_1.logger.warn("Не удалось уведомить пользователей о важном сообщении", {
                ...
                logger_1.logger.error("Не удалось обработать задачи из сообщения", {
        ...
        try {
            const createdTasks = await taskService_1.taskService.processIncomingMessage(message);
            if (createdTasks.length > 0) {
                const response = [
                    "Нашёл потенциальные задачи:",
                    (0, text_1.formatBulletList)(createdTasks.map((task) => {
                        const due = task.dueDate ? `дедлайн ${(0, date_1.formatDate)(task.dueDate)}` : "без срока";
    ...
    async handleHelpCommand(ctx) {
        ...
        try {
            ...
            const keyboard = keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null);
            if (isPersonal) {
                const senderUserId = (0, number_1.toInt)(ctx.message?.sender?.user_id);
                if (senderUserId) {
                    await this.bot.api.sendMessageToUser(senderUserId, text, { attachments: [keyboard] });
                }
                else {
                    logger_1.logger.warn("Не найден userId, используется ctx.reply", {
            ...
            logger_1.logger.success("Справка отправлена", { userId });
        }
        catch (error) {
            logger_1.logger.error("Ошибка отправки справки", {
                userId,
                action: "help",
                location: "handleHelpCommand",
                error,
     ...
     */
    async getChatIdForCommand(ctx) {
        const userId = (0, number_1.toInt)(ctx.user?.user_id);
        const isPersonal = ctx.message.recipient.chat_type === "dialog";
        // If command is from a group chat, use that chat and auto-add to user's list
        const contextChatId = (0, number_1.toInt)(ctx.chatId);
        if (contextChatId && !isPersonal) {
            // Auto-add group chat to user's list if not already there
            if (userId) {
                try {
                    const chatTitle = ctx.chat?.title ?? undefined;
                    await userChatService_1.userChatService.addChat(userId, contextChatId, chatTitle);
                    // Auto-select this chat as active
                    await userChatService_1.userChatService.selectChat(userId, contextChatId);
                }
                catch (error) {
                    logger_1.logger.warn("Не удалось автоматически добавить/выбрать чат", {
                        userId,
        ...
        const selectedChatId = await userChatService_1.userChatService.getSelectedChat(userId);
        if (selectedChatId) {
            const numericChatId = (0, number_1.toInt)(selectedChatId);
     ...
     */
    async getActiveChatInfo(userId) {
        const selectedChatId = await userChatService_1.userChatService.getSelectedChat(userId);
        if (!selectedChatId) {
            return null;
        }
        const userChats = await userChatService_1.userChatService.getUserChats(userId);
        const selectedChat = userChats.find((c) => c.chatId === selectedChatId);
        return selectedChat
            ? { id: selectedChatId, title: selectedChat.chatTitle }
            : { id: selectedChatId, title: null };
    }
    async handleDigestCommand(ctx) {
        try {
            const userId = (0, number_1.toInt)(ctx.user?.user_id);
            logger_1.logger.command(userId ?? undefined, "digest", ctx.chatId);
            ...
            if (!range) {
                await ctx.reply("Не понял период. Используйте: /digest сегодня|вчера|неделя|2025-11-01|2025-11-01:2025-11-03");
                return;
            }
            ...
            if (!chatId) {
                ...
                if (isPersonal && userId) {
                    ...
                    await ctx.reply(text, { attachments: [keyboardService_1.keyboardService.getChatsMenu()] });
                }
                else {
                    await ctx.reply("Не удалось определить чат. Используйте /select_chat для выбора чата или вызовите команду из нужного чата.");
                }
                return;
            }
            const chatTitle = ctx.chat?.title ?? "Учебный чат";
            const audienceId = (0, number_1.toInt)(ctx.user?.user_id);
            const digestOptions = audienceId ? { audienceUserId: audienceId } : undefined;
            logger_1.logger.userAction(userId ?? undefined, "Генерация дайджеста", { chatId, chatTitle });
            const summary = await digestService_1.digestService.generateDigest(chatId, chatTitle, range, digestOptions ?? {}, this.bot.api);
            // Отправляем дайджест с markdown форматированием
            await ctx.reply(summary, { format: "markdown" });
            logger_1.logger.success("Дайджест сгенерирован и отправлен", { userId, chatId: String(chatId) });
        }
        catch (error) {
            logger_1.logger.error("Ошибка генерации дайджеста", {
                userId: (0, number_1.toInt)(ctx.user?.user_id),
                action: "digest",
    ...
    async handleDeadlinesCommand(ctx) {
        const chatId = await this.getChatIdForCommand(ctx);
        if (!chatId) {
            const userId = (0, number_1.toInt)(ctx.user?.user_id);
            const isPersonal = ctx.message?.recipient?.chat_type === "dialog";
        ...
        const tasks = await taskService_1.taskService.getUpcomingTasks(chatId, (0, date_1.addDays)(new Date(), 7));
        if (tasks.length === 0) {
            ...
            if (ctx.update?.update_type === "message_callback") {
                await ctx.answerOnCallback({
                    message: { text, attachments: [keyboardService_1.keyboardService.getBackMenu()] },
                });
            }
            else {
        ...
        const summary = (0, text_1.formatBulletList)(tasks.map((task) => {
            const parts = [task.title];
            if (task.dueDate)
                parts.push(`дедлайн ${(0, date_1.formatDate)(task.dueDate)}`);
    ...
    async handleCalendarCommand(ctx) {
        const userId = (0, number_1.toInt)(ctx.user?.user_id);
        if (!userId) {
        ...
        try {
            ...
            const userTasks = await taskService_1.taskService.getPersonalTasks(userId, (0, date_1.addDays)(new Date(), 60));
            if (userTasks.length === 0) {
                await ctx.reply("📅 У вас пока нет задач с дедлайнами.\n\nЗадачи будут автоматически добавлены в календарь, когда появятся дедлайны в чатах.", { attachments: [keyboardService_1.keyboardService.getBackMenu()] });
                return;
            }
            ...
            userTasks.forEach((task) => {
                if (task.dueDate) {
                    const dateStr = (0, date_1.formatDate)(task.dueDate, "Europe/Moscow");
            ...
            calendarText.push(`\n**Ближайший дедлайн:** ${(0, date_1.formatDate)(userTasks[0]?.dueDate ?? new Date(), "Europe/Moscow")}`);
            // Пытаемся экспортировать в ICS
            const calendar = await calendarService_1.calendarService.exportUserCalendar(userId);
            if (calendar) {
                calendarText.push(`\n\n💡 *Календарь можно экспортировать в формате ICS через мини-приложение.*`);
            }
            await ctx.reply(calendarText.join("\n"), {
                format: "markdown",
                attachments: [keyboardService_1.keyboardService.getCalendarMenu()]
            });
        }
        catch (error) {
            logger_1.logger.error("Ошибка генерации календаря", {
                userId,
    ...
    async handleSearchCommand(ctx) {
        const userId = (0, number_1.toInt)(ctx.user?.user_id);
        logger_1.logger.command(userId ?? undefined, "search", ctx.chatId);
        const chatId = await this.getChatIdForCommand(ctx);
        if (!chatId) {
            const isPersonal = ctx.message?.recipient?.chat_type === "dialog";
            if (isPersonal && userId) {
        ...
        if (!query) {
            await ctx.reply("🔍 Поиск по материалам и сообщениям", {
                attachments: [keyboardService_1.keyboardService.getBackMenu()],
            });
            return;
        }
        try {
            ...
            const [materials, messages] = await Promise.all([
                searchService_1.searchService.searchMaterials(chatId, query, 10),
                searchService_1.searchService.searchMessages(chatId, query, 10),
            ]);
            if (materials.length === 0 && messages.length === 0) {
                await ctx.reply(`По запросу «${query}» ничего не найдено.\n\nПопробуйте изменить запрос или проверьте, что выбран правильный чат.`, { attachments: [keyboardService_1.keyboardService.getBackMenu()] });
                return;
            }
            const results = [];
            ...
            if (messages.length > 0) {
                ...
                messages.forEach((m, index) => {
                    const text = (0, text_1.sanitizeText)(m.text ?? "");
                    const preview = text.length > 80 ? `${text.substring(0, 80)}...` : text;
                    const sender = m.senderName ?? "Участник";
                    const date = (0, date_1.formatDate)(m.timestamp);
                    results.push(`${index + 1}. ${sender} (${date}):`);
                    results.push(`   ${preview}`);
                });
            }
            await ctx.reply(results.join("\n"), { attachments: [keyboardService_1.keyboardService.getBackMenu()], format: "markdown" });
            logger_1.logger.userAction(userId ?? undefined, "Выполнен поиск", { chatId, query });
        }
        catch (error) {
            logger_1.logger.error("Ошибка выполнения поиска", {
                userId,
                chatId,
                query,
    ...
    async handleMaterialsCommand(ctx) {
        const chatId = await this.getChatIdForCommand(ctx);
        if (!chatId) {
            const userId = (0, number_1.toInt)(ctx.user?.user_id);
            const isPersonal = ctx.message?.recipient?.chat_type === "dialog";
        ...
        const materials = await searchService_1.searchService.getAllMaterials(chatId, 30);
        if (materials.length === 0) {
        ...
        const formattedMaterials = (0, text_1.formatMaterials)(materials);
        const text = [
        ...
        if (ctx.update?.update_type === "message_callback") {
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getBackMenu()], format: "markdown" },
            });
        }
        else {
    ...
    async handleTasksCommand(ctx) {
        ...
        const tasks = await taskService_1.taskService.getAllTasks(chatId, 30);
        if (tasks.length === 0) {
        ...
        await ctx.reply([
            ...
            (0, text_1.formatBulletList)(tasks.map((task) => {
                const parts = [task.title];
                if (task.dueDate) {
                    parts.push(`дедлайн ${(0, date_1.formatDate)(task.dueDate)}`);
                }
    ...
    async handleChatsCommand(ctx) {
        ...
        if (userChats.length === 0) {
            ...
            ].join("\n"), { attachments: [keyboardService_1.keyboardService.getChatsMenu()] });
            return;
        }
        const chatList = userChats.map((chat, index) => {
        ...
        const text = [
            "📋 Ваши чаты:",
            "",
            (0, text_1.formatBulletList)(chatList),
            "",
            "Выберите чат кнопкой ниже или используйте /select_chat <номер>.",
        ].join("\n");
        await ctx.reply(text, {
            attachments: [keyboardService_1.keyboardService.getChatSelectionMenu(chats, selectedChatIdNum)],
        });
    }
    async handleSelectChatCommand(ctx) {
        const userId = (0, number_1.toInt)(ctx.user?.user_id);
        if (!userId) {
            await ctx.reply("Команда доступна только авторизованным пользователям.");
            return;
        ...
        if (!Number.isNaN(chatIndex) && chatIndex > 0 && chatIndex <= userChats.length) {
            ...
            const chatId = (0, ids_1.ensureIdString)(chatNumberOrId);
            selectedChat = userChats.find((c) => c.chatId === chatId);
        }
        if (!selectedChat) {
        ...
        try {
            await userChatService_1.userChatService.selectChat(userId, selectedChat.chatId);
            await ctx.reply(`✅ Выбран чат: ${selectedChat.chatTitle ?? `Чат ${selectedChat.chatId}`}`);
        }
        catch (error) {
            logger_1.logger.error("Ошибка выбора чата", {
                userId,
                action: "select_chat",
    ...
    async handleSyncChatsCommand(ctx) {
        const userId = (0, number_1.toInt)(ctx.user?.user_id);
        if (!userId) {
            await ctx.reply("Команда доступна только авторизованным пользователям.");
            return;
        ...
        try {
            ...
            const count = await userChatService_1.userChatService.syncChatsFromMax(userId, {
                getAllChats: async () => {
                    const response = await this.bot.api.getAllChats();
                    return {
                        chats: response.chats?.map((chat) => ({
                            chat_id: (0, number_1.toInt)(chat.chat_id) ?? 0,
                            title: chat.title ?? undefined,
                        })),
                    };
                },
                getChatMembers: async (chatId, user_ids) => {
                    try {
            ...
            logger_1.logger.error("Ошибка синхронизации чатов", {
                userId,
                action: "sync_chats",
    ...
    resolveRange(args) {
        ...
        if (arg === "сегодня" || arg === "today") {
            return { from: (0, date_1.startOfDay)(), to: (0, date_1.endOfDay)() };
        }
        if (arg === "вчера" || arg === "yesterday") {
            const yesterday = (0, date_1.addDays)(new Date(), -1);
            return { from: (0, date_1.startOfDay)(yesterday), to: (0, date_1.endOfDay)(yesterday) };
        }
        if (arg === "неделя" || arg === "week") {
            return { from: (0, date_1.startOfWeek)(), to: (0, date_1.endOfWeek)() };
        }
        ...
        if (arg.includes(":")) {
            ...
            if (parts.length === 2) {
                ...
                if (fromParsed && toParsed) {
                    return { from: (0, date_1.startOfDay)(fromParsed), to: (0, date_1.endOfDay)(toParsed) };
                }
            }
        }
        // Парсинг одной даты
        ...
        if (parsed) {
            return { from: (0, date_1.startOfDay)(parsed), to: (0, date_1.endOfDay)(parsed) };
        }
        ...
        if (isoDate) {
            ...
            if (!Number.isNaN(date.getTime())) {
                return { from: (0, date_1.startOfDay)(date), to: (0, date_1.endOfDay)(date) };
            }
        }
        return null;
    }
    handleReminder = async (task, reminder) => {
        const userId = reminder.userId ?? task.assigneeId ?? task.createdByUserId;
        const messageLines = [
            "⏰ Напоминание о задаче:",
            task.title,
            task.dueDate ? `Дедлайн: ${(0, date_1.formatDate)(task.dueDate)}` : "Срок не указан.",
            task.description ? `Описание: ${task.description}` : "",
            `Источник сообщения: ${task.sourceMessageId}`,
        ].filter(Boolean);
        try {
            if (userId) {
                const numericUserId = (0, number_1.toInt)(userId);
                if (!numericUserId) {
                    logger_1.logger.error("Не удалось преобразовать userId в число для напоминания", {
                ...
                const numericChatId = (0, number_1.toInt)(task.chatId);
                if (!numericChatId) {
                    logger_1.logger.error("Не удалось преобразовать chatId в число для напоминания", {
                        chatId: task.chatId,
                        location: "handleReminder",
            ...
            logger_1.logger.error("Ошибка отправки напоминания", {
                location: "handleReminder",
                taskId: task.id,
    ...
    registerButtonHandlers() {
        ...
        this.bot.action("start", async (ctx) => {
            ...
            logger_1.logger.userAction(userId, "Кнопка 'Начать' нажата", { userName });
        });
        ...
        this.bot.action("action:start", async (ctx) => {
            ...
            logger_1.logger.userAction(userId, "Кнопка 'Начать' нажата (action:start)", { userName });
        });
        // Главное меню
        this.bot.action("action:main_menu", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null)] },
            });
        });
        // Помощь
        this.bot.action("action:help", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: {
                    text: helpText,
                    attachments: [keyboardService_1.keyboardService.getMainMenu(activeChat?.title ?? null)],
                },
            });
        });
        // Чаты
        this.bot.action("action:chats", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getChatsMenu()] },
            });
        });
        this.bot.action("action:chats_list", async (ctx) => {
            ...
            try {
                ...
                const syncCount = await userChatService_1.userChatService.syncChatsFromMax(userId, {
                    getAllChats: async () => {
                        const response = await this.bot.api.getAllChats();
                        return {
                            chats: response.chats?.map((chat) => ({
                    ...
                    getChatMembers: async (chatId, user_ids) => {
                        ...
                        return {
                            members: response.members?.map((m) => ({ user_id: (0, number_1.toInt)(m.user_id) ?? 0 })),
                        };
                    },
                });
                ...
                await ctx.answerOnCallback({
                    message: { text, attachments: [keyboardService_1.keyboardService.getChatSelectionMenu(chats, selectedChatIdNum)] },
                });
            }
            catch (error) {
                logger_1.logger.error("Ошибка синхронизации чатов", {
        ...
        this.bot.action("action:sync_chats", async (ctx) => {
            ...
            try {
                ...
                logger_1.logger.error("Ошибка синхронизации чатов из кнопки", {
                    userId: (0, number_1.toInt)(ctx.user?.user_id),
        ...
        this.bot.action(/^action:select_chat:(-?\d+)$/, async (ctx) => {
            ...
            try {
                ...
                await userChatService_1.userChatService.selectChat(userId, chatIdStr);
                const userChats = await userChatService_1.userChatService.getUserChats(userId);
                const selectedChat = userChats.find((c) => c.chatId === chatIdStr);
                ...
                logger_1.logger.error("Ошибка выбора чата из кнопки", {
                    userId,
                    chatId: chatIdStr,
                    action: "select_chat",
        ...
        this.bot.action("action:tasks", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getTasksMenu()] },
            });
        });
        this.bot.action("action:tasks_list", async (ctx) => {
            ...
            const chatId = await userChatService_1.userChatService.getSelectedChat(userId);
            if (!chatId) {
                await ctx.answerOnCallback({
            ...
            const tasks = await taskService_1.taskService.getAllTasks(numericChatId, 30);
            if (tasks.length === 0) {
                await ctx.answerOnCallback({
                    message: { text: "В чате пока нет задач.", attachments: [keyboardService_1.keyboardService.getBackMenu()] },
                });
                return;
            }
            const summary = (0, text_1.formatBulletList)(tasks.map((task) => {
        ...
        this.bot.action("action:tasks_week", async (ctx) => {
            ...
            const tasks = await taskService_1.taskService.getUpcomingTasks(numericChatId, (0, date_1.addDays)(new Date(), 7));
            if (tasks.length === 0) {
            ...
            await ctx.answerOnCallback({
                message: { text: `📅 Задачи на неделю:\n\n${summary}`, attachments: [keyboardService_1.keyboardService.getBackMenu()] },
            });
        });
        this.bot.action("action:tasks_tomorrow", async (ctx) => {
            ...
            const tomorrow = (0, date_1.addDays)(new Date(), 1);
            const tasks = await taskService_1.taskService.getUpcomingTasks(numericChatId, (0, date_1.endOfDay)(tomorrow));
            if (tasks.length === 0) {
        ...
        this.bot.action("action:deadlines", async (ctx) => {
            ...
            if (tasks.length === 0) {
                await ctx.answerOnCallback({
                    message: { text: "На ближайшую неделю дедлайнов не найдено.", attachments: [keyboardService_1.keyboardService.getBackMenu()] },
                });
                return;
            }
            const summary = (0, text_1.formatBulletList)(tasks.map((task) => {
        ...
        this.bot.action("action:materials", async (ctx) => {
            ...
            const materials = await searchService_1.searchService.getAllMaterials(numericChatId, 30);
            if (materials.length === 0) {
                await ctx.answerOnCallback({
                    message: { text: "В чате пока нет материалов.", attachments: [keyboardService_1.keyboardService.getBackMenu()] },
                });
                return;
            }
            // Используем функцию форматирования материалов для единообразия
        ...
        this.bot.action("action:digest", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getDigestMenu()] },
            });
        });
        this.bot.action("action:digest_period", async (ctx) => {
            await ctx.answerOnCallback({
                message: {
                    ...
                    attachments: [keyboardService_1.keyboardService.getDigestMenu()],
                },
            });
        });
        this.bot.action("action:digest_today", async (ctx) => {
            ...
            const fromDate = (0, date_1.startOfDay)(today);
            const toDate = (0, date_1.endOfDay)(today);
            // Get chat title
            const userChats = await userChatService_1.userChatService.getUserChats(userId);
            const selectedChat = userChats.find((c) => c.chatId === chatId);
            const chatTitle = selectedChat?.chatTitle ?? `Чат ${chatId}`;
            const digest = await digestService_1.digestService.generateDigest(numericChatId, chatTitle, { from: fromDate, to: toDate }, {}, this.bot.api);
            if (!digest) {
                await ctx.answerOnCallback({ notification: "Дайджест за сегодня пуст" });
                return;
            }
            await ctx.answerOnCallback({
                message: { text: digest, attachments: [keyboardService_1.keyboardService.getBackMenu()] },
            });
        });
        this.bot.action("action:digest_week", async (ctx) => {
            ...
            const now = new Date();
            const fromDate = (0, date_1.startOfWeek)(now);
            const toDate = (0, date_1.endOfWeek)(now);
            // Get chat title
            const userChats = await userChatService_1.userChatService.getUserChats(userId);
            const selectedChat = userChats.find((c) => c.chatId === chatId);
            const chatTitle = selectedChat?.chatTitle ?? `Чат ${chatId}`;
        ...
        this.bot.action("action:calendar_export_excel", async (ctx) => {
            ...
            try {
                ...
                const excelResult = await calendarService_1.calendarService.exportUserCalendarToExcel(userId);
                if (!excelResult) {
                    await ctx.answerOnCallback({
                        message: {
                ...
                const tempFilePath = (0, node_path_1.join)((0, node_os_1.tmpdir)(), excelResult.filename);
                (0, node_fs_1.writeFileSync)(tempFilePath, excelResult.buffer);
                try {
                    ...
                    const fileAttachment = new max_bot_api_1.FileAttachment({ token: uploadedFile.token });
                    const text = [
                    ...
                    const senderUserId = (0, number_1.toInt)(ctx.user?.user_id);
                    if (senderUserId) {
                        await ctx.api.sendMessageToUser(senderUserId, text, {
                            attachments: [fileAttachment.toJson()],
                            format: "markdown",
                        });
                        logger_1.logger.success("Excel файл экспортирован и отправлен", { userId: senderUserId });
                    }
                    else {
                        await ctx.answerOnCallback({
                            message: {
                    ...
                    logger_1.logger.error("Ошибка экспорта календаря в Excel", {
                        userId,
                    ...
                    try {
                        (0, node_fs_1.unlinkSync)(tempFilePath);
                    }
                    catch (cleanupError) {
                        logger_1.logger.warn("Не удалось удалить временный файл", {
                ...
                logger_1.logger.error("Ошибка при генерации Excel файла", {
                    userId,
                    location: "action:calendar_export_excel",
                    error,
                });
        ...
        this.bot.action("action:settings", async (ctx) => {
            ...
            await ctx.answerOnCallback({
                message: { text, attachments: [keyboardService_1.keyboardService.getSettingsMenu()] },
            });
        });
        this.bot.action("action:settings_reminders", async (ctx) => {
            ...
            const preference = await preferenceService_1.preferenceService.getOrCreate((0, ids_1.ensureIdString)(userId));
            await ctx.answerOnCallback({
                message: {
                    text: [
