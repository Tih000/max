...
graph TB
    ...
    subgraph "Service Layer"
        ...
        L[User Chat Service]
        M[Material Service]
        N[Important Message Service]
        O[Keyboard Service]
        P[Scheduled Digest Service]
        Q[Preference Service]
    end
    
    subgraph "Infrastructure Layer"
        R[(PostgreSQL<br/>Prisma ORM)]
        S[Node Schedule<br/>Планировщик]
        T[Logger<br/>Pino]
    end
    
    subgraph "AI Integration"
        U[GigaChat Service<br/>LLM Wrapper]
    end
...
- **Material Service**: Извлечение и анализ материалов (через Message Service)
- **Important Message Service**: Определение важных сообщений и уведомление пользователей
- **Keyboard Service**: Генерация интерактивных клавиатур для UI
- **Scheduled Digest Service**: Планирование и автоматическая отправка дайджестов
- **Preference Service**: Управление настройками пользователя
...
### 16.3. Автоматическое тестирование
-  **Jest** для unit-тестирования
-  **49 автоматических тестов** покрывают:
  - Утилиты (text, date, number) - 42 теста
  - Критичные сервисы (taskService, digestService) - 7 тестов
-  Все тесты проходят успешно
-  Время выполнения: ~6-13 секунд
-  Структура тестов: `src/**/__tests__/*.test.ts`

---

## 17. ТЕСТИРОВАНИЕ И КАЧЕСТВО КОДА

### 17.1. Автоматическое тестирование

Проект включает автоматические тесты на Jest:

**Структура тестов:**
- `src/utils/__tests__/` - unit-тесты утилит
- `src/services/__tests__/` - unit-тесты сервисов

**Покрытие:**
- ✅ 49 автоматических тестов
- ✅ 5 test suites (все проходят)
- ✅ Утилиты: полное покрытие (text, date, number)
- ✅ Сервисы: основные методы (taskService, digestService)
...
-  **Качество кода**: Автоматическое тестирование, структурированное логирование
